...
def _get_settings():
    return {
        ...
        "winners_pct": bot_config.get("pool_lottery_winners_pct", 10),
        "max_tickets": bot_config.get("pool_lottery_max_tickets", 10),
    }
...
def draw_pool_lottery():
    ...
    winners_count = max(1, math.ceil(len(tickets) * settings["winners_pct"] / 100))
    winners_count = min(winners_count, len(tickets))
...
async def handle_buy_pool_ticket(message, args):
    ...
    conn = get_db()
    my_count = conn.execute("SELECT COUNT(*) as c FROM pool_lottery_tickets WHERE user_id = ? AND round = ?",
                            (user["id"], current_round)).fetchone()["c"]
    conn.close()
    if my_count >= settings["max_tickets"]:
        await message.reply(content=f"每人最多持有 {settings['max_tickets']} 张奖池彩彩票，当前已持有 {my_count} 张")
        return
    ...
    winners = max(1, math.ceil((ticket_count + 1) * settings["winners_pct"] / 100))
    await message.reply(content=f"购买成功！第{current_round}期 奖池彩\n票价: {settings['ticket_price']}，当前奖池: {new_pool:.2f}\n中奖比例: {settings['winners_pct']}%（约{winners}人），余额: {new_balance:.2f}")
...
async def handle_pool_lottery_info(message, args):
    ...
    winners = max(1, math.ceil(ticket_count * settings["winners_pct"] / 100)) if ticket_count > 0 else 1
    lines = [
        f"=== 奖池彩 第{current_round}期 ===",
        f"票价: {settings['ticket_price']}",
        f"奖池: {pool:.2f}",
        f"已售: {ticket_count}张",
        f"中奖比例: {settings['winners_pct']}%（约{winners}人）",
    ]

    if ticket_count > 0 and winners > 0:
        lines.append(f"预估每人奖金: {round(pool / winners, 2):.2f}")
