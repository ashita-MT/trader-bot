...
def _get_settings():
    return {
        "ticket_price": bot_config.get("number_lottery_ticket_price", 100),
        "prize_amount": bot_config.get("number_lottery_prize_amount", 10000),
        "max_tickets": bot_config.get("number_lottery_max_tickets", 10),
    }


def get_current_round():
    conn = get_db()
    row = conn.execute("SELECT MAX(round) as r FROM number_lottery_draws").fetchone()
    conn.close()
    return (row["r"] or 0) + 1
...
def get_last_draw():
    conn = get_db()
    row = conn.execute("SELECT * FROM number_lottery_draws ORDER BY round DESC LIMIT 1").fetchone()
    conn.close()
...
def draw_number_lottery():
    ...
    conn = get_db()
    conn.execute("INSERT INTO number_lottery_draws (round, winning_number) VALUES (?, ?)", (current_round, winning_number))

    winners = conn.execute("""
        SELECT t.user_id, u.balance
        FROM number_lottery_tickets t
    ...
    conn.execute("DELETE FROM number_lottery_tickets WHERE round = ?", (current_round,))
    conn.commit()
    conn.close()
    return {"round": current_round, "winning_number": winning_number, "winners": len(winners)}


async def handle_buy_number_lottery(message, args):
    ...
    conn = get_db()
    my_count = conn.execute("SELECT COUNT(*) as c FROM number_lottery_tickets WHERE user_id = ? AND round = ?",
    ...
    conn.execute("INSERT INTO number_lottery_tickets (user_id, number, round) VALUES (?, ?, ?)",
...
async def handle_my_number_tickets(message, args):
    ...
    current_round = get_current_round()
    tickets = conn.execute("SELECT number FROM number_lottery_tickets WHERE user_id = ? AND round = ?",
                           (user["id"], current_round)).fetchall()
    conn.close()

    if not tickets:
        await message.reply(content=f"第{current_round}期 你还没有号码彩彩票")
        return
